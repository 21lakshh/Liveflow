...
const Bar = memo<{
  ...
  <div
    ...
    className={cn(
      "max-w-[12px] min-w-[6px] flex-1 transition-all duration-150",
      "rounded-full",
      "bg-border data-[highlighted=true]:bg-primary",
      // Add glow when text-primary is used in speaking state
      state === "speaking" && "bg-primary shadow-[0_0_15px_hsl(var(--accent-glow)/0.5)]",
      // Thinking state gets a customized pulse and glow
      isHighlighted && "shadow-[0_0_12px_hsl(var(--accent-glow)/0.4)]",
      state === "thinking" && isHighlighted && "animate-pulse shadow-[0_0_20px_hsl(var(--accent-glow)/0.6)]"
    )}
    style={{
      height: `${heightPct}%`,
